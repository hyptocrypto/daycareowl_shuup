...
from daycare_owl.models.daycare import DayCare
...
class Picotable(object):
    ...
    def process_queryset(self, query):
        # NOTE This little hack is so that suppliers can be filterd based off daycare.verifed boolean
        if "is_verified" in query["filters"].keys():
            if isinstance(query["filters"].get("is_verified"), bool):
                is_verified = query["filters"].pop("is_verified")
                suppliers_filted_by_daycare_verifed = [d.daycare.pk for d in DayCare.objects.filter(verified=is_verified)]
                self.queryset = self.queryset.filter(pk__in=suppliers_filted_by_daycare_verifed)
            else:
                query["filters"].pop("is_verified")
    ...
    def get_data(self, query):
        if query.get("sort") and "is_verified" in query["sort"]:
            query["sort"] = ""
